...
def prediction(training_file, test_file, features_on, output_file=''):
    results = []
    for x in TARGETS_NAMES:
        training_set, output, test_set = get_all(training_file, test_file, features_on[x])
        results.append((lambda f: f(training_set, output[x], test_set))(predict_clf if TARGETS[x] == 'cls' else predict_lin)) 
